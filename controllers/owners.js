...
app.post("/register", upload.none(),async (req,res)=>{
    try{
        ...
        const {username, first_name,last_name,email,password,contact_no} = req.body;
        if(!(username, first_name,last_name,email,password,contact_no)){
...
app.get("/pending-sessions",sessionCheck.isOwner, upload.none(),async (req,res)=>{
   try{
       ...
       if(psession.length<1){
           ...
           res.status(200).json({status:200,message:"Pending sessions data", success:true, data:psession});
       }
   } catch(err){
       console.log(err);
       res.status(500).json({status:500,message:"Internal server error",success:false,data:null})
...
app.get("/complete-matches", sessionCheck.isOwner, upload.none(), async (req,res)=>{
   try{
       ...
       res.status(200).json({status:200,message:"Total matches completed : "+matches.length, success:true,data:matches});
       }
   } catch(err){
       console.log(err);
       res.status(500).json({status:500, message:"Internal server error", success:false, data:err});
...
app.get("/inplay-matches", sessionCheck.isOwner, upload.none(), async (req,res)=>{
   try{
       ...
       res.status(200).json({status:200,message:"Total matches in-play : "+matches.length, success:true,data:matches});
       }
   } catch(err){
       console.log(err);
       res.status(500).json({status:500, message:"Internal server error", success:false, data:err});
...
app.get("/upcoming-matches", sessionCheck.isOwner, upload.none(), async (req,res)=>{
   try{
       ...
       res.status(200).json({status:200,message:"Total upcoming matches : "+matches.length, success:true,data:matches});
       }
   } catch(err){
       console.log(err);
       res.status(500).json({status:500, message:"Internal server error", success:false, data:null});
...
app.get("/total-clients", sessionCheck.isOwner, upload.none(), async (req,res)=>{
    try{
        ...
        if(clients.length<1){
            ...
            res.status(200).json({status:200,message:"Total clients : "+clients.length, success:true, data:clients});
        }
...
app.get("/total-agents", sessionCheck.isOwner, upload.none(), async (req,res)=>{
    try{
        ...
        if(agents.length<1){
            ...
            res.status(200).json({status:200,message:"Total agents : "+agents.length, success:true, data:agents});
        }
...
app.get("/total-stokists", sessionCheck.isOwner, upload.none(), async (req,res)=>{
    try{
        ...
        if(stokists.length<1){
            ...
            console.log(stokists.length);
            res.status(200).json({status:200,message:"Total stokists : "+stokists.length, success:true, data:stokists});
        }
...
app.get("/total-seniorStokists", sessionCheck.isOwner, upload.none(), async (req,res)=>{
    try{
        ...
        if(seniorStokists.length<1){
            ...
            console.log(seniorStokists.length);
            res.status(200).json({status:200,message:"Total senior stokists : "+seniorStokists.length, success:true, data:seniorStokists});
        }
...
app.get("/total-admins", sessionCheck.isOwner, upload.none(), async (req,res)=>{
    try{
        ...
        if(admins.length<1){
            ...
            res.status(200).json({status:200,message:"Total admins : "+admins.length, success:true, data:admins});
        }
...
app.get("/manual-matches",sessionCheck.isOwner, upload.none(), async (req,res)=>{
   try{
       ...
       res.status(200).json({status:200,message:"Total matches inserted manually : "+matches.length, success:true,data:matches});
       }
   } catch(err){
       console.log(err);
       res.status(500).json({status:500, message:"Internal server error", success:false, data:err});
...
app.get("/api-matches",sessionCheck.isOwner, upload.none(), async (req,res)=>{
   try{
       ...
       res.status(200).json({status:200,message:"Total matches inserted by API : "+matches.length, success:true,data:matches});
       }
   } catch(err){
       console.log(err);
       res.status(500).json({status:500, message:"Internal server error", success:false, data:err});
...
app.get("/abandoned-matches", sessionCheck.isOwner, upload.none(), async (req,res)=>{
   try{
       ...
       if(matches.length<1){
           ...
           res.status(200).json({status:200,message:"Total matches abandoned : "+matches.length, success:true,data:matches});
       }
   } catch(err){
       console.log(err);
       res.status(500).json({status:500, message:"Internal server error", success:false, data:err});
...
app.post("/remove-match",sessionCheck.isOwner,upload.none(), async (req,res)=>{
   try{
       const market_id = req.body.market_id;
       console.log(market_id);
       await Match.updateOne({market_id},{$set:{status:"removed"}}).exec();
       const match = await Match.findOne({market_id});
       console.log(match);
...
app.post("/match-setting", sessionCheck.isOwner, upload.none(), async (req,res)=>{
   try{
       ...
       await Match.updateOne({market_id},{$set:{odds_type:odds_type,insert_type:insert_type,status:status,match_type:match_type,session_api_hit:session_api_hit,fancy_running_type:fancy_running_type,
              ...
              score_id,funting_status,fav_team,maximum_match_amount,minimum_match_amount,
              maximum_session_amount:maximum_session_amount,minimum_session_amount:minimum_session_amount,team1:team1,team2:team2}}).exec();
